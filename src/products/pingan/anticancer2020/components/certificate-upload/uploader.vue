...
export default {
  props: {
    ...
    uploadUrl: { // 上传地址， 即：服务器地址
      type: String,
      default: ''
    },
    size: { // 图片大小类型  normal or small
      type: String,
      default: 'normal'
    },
    capture: { // 暂时无用
  ...
      return new File([u8arr], filename, {type:mime});
    ...
    // 上传file文件至文件服务器
    uploadImage(image) {
      let file = this.dataURLtoFile(image, '1.jpg');
    ...
    // 上传图片，如果图片大于1m，则进行压缩，再上传，如果小于500k，则直接上传
    change () {
      if (this.handleClick) {
        return;
      }

      let maxSize = 500 * 1024;   //500k
      ...
      fr.onload = function () {
        ...
        image.onload = function() {
          if (imageUrl.length < maxSize) {
            self.uploadImage(imageUrl); // 若小于最大尺寸限制， 直接调取上传
          } else {
            self.uploadImage(self.compress(image)); // 若大于最大限制， 则先调取压缩图片方法再调取上传方法
          }
        }
