<template>
    <div id="abc">
      <div class="J-weixin-tip weixin-tip" id="weixin-tip">
      ...
      <div class="footer" v-if="isShow">
          <div class="fixed-footer-quxiao" @click="confirmcancel">支付失败</div>
          <div class="fixed-footer-insure" @click="confirmCod">支付完成</div>
      </div>
      <div class="footer-text" v-if="isShow">
        <p>温馨提示</p>
        <p>. 如果未`打开APP或者未完成付款，请按照提示再次进行</p>
        <p>. 如果你已经完成付款，请点击"支付完成"</p>
        <p>. 如果无法完成支付，请点击返回，选择微信支付`</p>
      </div>
      <!-- <jn-dialog v-show="conShow" conTitle="支付确认" :close="confirmCod" :enterFun="confirmCod">
        <div>
          <p>请在浏览器中完成支付</p>
        </div>
      </jn-dialog> -->
...
import _AP from '@/views/pay/alipay/ap.js'
// import jnDialog from '@/components/widget/jn-dialog'
export default {
  components: {
    // jnDialog
  },
  ...
  methods: {
    ...
    confirmCod () {
      let order_sn = window.sessionStorage.getItem('ali_order_sn')
      window.location.href = `${location.protocol}//${location.hostname}${location.port ? ':' + location.port : ''}/vue/#/pay/process?order_sn=${order_sn}`
      // this.$router.replace({name: 'pay_process', params: {order_sn: order_sn}})
    },
    confirmcancel () {
      history.go(-2)
    }
  ...
  mounted () {
    if (location.hash.indexOf('error') !== -1) {
      ...
      if (ua.indexOf('micromessenger') !== -1) {
        ...
        location.href = param !== '' ? AP.b.decode(param) : 'pay.htm#error'
      }
    }
  }
}
...
<style>
#abc {position: relative;z-index: 1000;height: 100%;background: #FFF;}
    .weixin-tip {
        display: none;
...
.fixed-footer-insure {
  ...
  background: url(../../../assets/image/sinosig/wddg/relay-bg.png) no-repeat;
  background-size: cover;
  font-size: .426667rem;
  color: #fff;
  text-align: center;
